sap.ui.controller("regtest.CONTROLLER.RegTestDetail", {
	...
	refreshRelatedTables : function() {
		...
		var filterCheck = new sap.ui.model.Filter("id_reg_test",
				sap.ui.model.FilterOperator.EQ, idRegTest);
	...
	onAddPlaceClick : function() {
		...
		fragAddPH = sap.ui.xmlfragment("regtest.fragments.addDialog", oView
				.getController());
		// }
		fragAddPH.open();
	},
	 ...
	 * onCloseDialog : function () { this._getDialog().close(); }, _getDialog :
	 * function () { // create dialog lazily if (!this._oDialog) { // create
	 * dialog via fragment factory this._oDialog =
	 * sap.ui.xmlfragment("com.tutorial.fragments.addDialog", this); // connect
	 * dialog to view (models, lifecycle)
	 * this.getView().addDependent(this._oDialog); } return this._oDialog; }
	 */
